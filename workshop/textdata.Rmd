...
As you can see above, only the first "o" is replaced by "e". To correct that, we would use `str_replace_all()`:
...
Regular expression is the common way to describe strings to any computer language. In the examples, I will speak in terms of what `str_extract()` will output from the string **"aabc123d"**
...
For example, if we use `string_extract_all()` with `..` we will get "aa", "ab", "bc", "c1", "12", "23", "3d". But if we use `..$` instead, we will only get "3d".
...
5. How do I make sure that every cohort name only has the first word?
```{r}
data%>%
