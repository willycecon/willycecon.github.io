...
1. For this question, we will be using the same data set from the first quesion in the ggplot2 workshop. You can load the data with `readr::read_csv()` with the path `https://willythewoo.github.io/WillyTheWoo/workshop/data/acs_sample.csv`.
...
2. This problem will teach you how to create a simulated data set under the generalized Roy model framework. It is also a good practice for understanding `tidyverse`. The three functions in i. are normal distribution, binomial distribution, and uniform distribution respetively.
