...
## Key and Value
From my discussion above, you are probably thinking that the line betwen long and wide seems blurry. You would be correct. A data set can actually be long and wide at the same time depending on how you want to define the **observation** and **instance**. 

Under `tidyverse`, we call these observations *"key"* and these instances *"value"*. Key tells R which attribute(s) uniquely identifies your observations. For example, in our workshop data, you can see the column `good` and the columns `bad1` through `bad9`. The variable good stores information on the observation's favorite week(s) of the perspectives class. Tha variables bad1 through bad9 store information on the observation's least favorite week(s) of class [unranked].
...
As you can see, if we look at the variable good, the data set is in  long form since every row represents a unique combination of student and favorite week. On the other hand, if we look at bad1 through bad9, each row for the sepcific student will have the same bad1 through bad9.
...
In `ggplot2`, we briefly talked about different types of graphs. Scatter plot, jitter plot, line plot, fitted-line plot, bar chart, histogram, box plot, etc. Each of these graphs has a `geom_...()` funtion associated with it. 
...
For more information on how to use these funcitons, check out the [ggplot2 cheat sheet](https://github.com/rstudio/cheatsheets/raw/master/data-visualization-2.1.pdf)
